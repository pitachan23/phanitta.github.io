...
private:
	...
	std::string nextPlayer; // "computer1" or "computer2"
	std::string compPick1, compPick2;
	...
	void printResult();
	int getRandMove();
	int getBestMove(std::string compPick);
	int checkTwoOcc(std::string compPick);
	int checkTwoEmpty(std::string compPick);
